...
export async function fetchMatchesFromApi(): Promise<MatchCard[]> {
  ...
  if (!response.ok) {
    throw new Error(`RapidAPI matches request failed: ${response.status} ${response.statusText}`);
  }

  const data = await response.json();
  const items = data?.response?.items || [];

  return items.map((m: RawMatch) => normalizeMatchCard(m));
}
